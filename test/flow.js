var HNYToken = artifacts.require("./HNYToken.sol");
var Crowdsale = artifacts.require("./HNYCrowdsale.sol");


var PERIOD_14_DAYS = 14*24*60*60;

contract('flow', function(accounts) {
...
//    console.log("Buyer balance", web3.fromWei(buyerBalance, "ether").toString(), " ETHER");
    ...
    return HNYToken.deployed().then(function(instance) {
      return instance.balanceOf.call(owner)
    .then(function(balance) {
      console.log("Owner balance: ", web3.fromWei(ownerBalance, "ether").toString(), " ETHER / ", balance.valueOf(), " HNY");
      return instance.balanceOf.call(buyer);
    }).then(function(balance) {
      console.log("Buyer balance: ", web3.fromWei(buyerBalance, "ether").toString(), " ETHER / ", balance.valueOf(), " HNY");
      return instance.balanceOf.call(wallet);
    }).then(function(balance) {
      console.log("Wallet balance: ", web3.fromWei(walletBalance, "ether").toString(), " ETHER / ", balance.valueOf(), " HNY");
    })
...
/*
  it("should put 1,000,000,000.000000 HNYToken in the owner account", function() {
  ...
  it("Send 600,000,000.000000 HNYToken to Crowdsale contract", function() {
    return HNYToken.deployed().then(function(coin) {
...
*/
  ...
  it("Buy 9 ETH coins on presale", function() {

    //web3.evm.increaseTime(1551992708);

    var investSum = web3.toWei(9, "ether");
    ...
    return Crowdsale.deployed().then(function(crowd) {
       return crowd.sendTransaction({from: buyer, to: crowd.address, value: investSum}).then(function(txn) {
         ...
         printBalance();
         assert.equal(balance.valueOf(), 75600, "75600 wasn't in the first account.");
     });
  });

  it("Buy 10 ETH coins on presale, volume discount applied of 10%", function() {

    //web3.evm.increaseTime(1551992708);

    var investSum = web3.toWei(10, "ether");
    ...
    return Crowdsale.deployed().then(function(crowd) {
       ...
       printBalance();
       assert.equal(balance.valueOf(), 168000, "168000 wasn't in the first account.");
     });
  });

  it("Buy 20 ETH coins on sale, volume discount applied of 20%", function() {

    web3.evm.increaseTime(30403200);

    var investSum = web3.toWei(20, "ether");
    ...
    return Crowdsale.deployed().then(function(crowd) {
       ...
       printBalance();
       assert.equal(balance.valueOf(), 312000, "312000 wasn't in the first account.");
     });
  });

   it("set 10k custom rate", async () => {
     let csale = await Crowdsale.deployed();
     csale.rateUpdate(10000, {from: owner});
     //csale.finalize({from: owner});
    })


  it("Buy 1 ETH coins on sale, custom discount 10k", function() {

    var investSum = web3.toWei(1, "ether");
    ...
    return Crowdsale.deployed().then(function(crowd) {
       ...
       printBalance();
       assert.equal(balance.valueOf(), 322000, "312000 wasn't in the first account.");
     });
  });
